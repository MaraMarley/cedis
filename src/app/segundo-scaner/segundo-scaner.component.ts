...
@Component({
  selector: 'app-segundo-scaner',
  imports: [ReactiveFormsModule, CommonModule],
  templateUrl: './segundo-scaner.component.html',
  styleUrl: './segundo-scaner.component.css'
})
export class SegundoScanerComponent {
    ...
    public onEnter(input: HTMLInputElement): void {
      ...
      if (!valor) {
        ...
        this.procesosCedisService.segundoEscaneo(valor).subscribe(resp => {          
    ...
    handleKeyboardEvent(event: KeyboardEvent): void {
      ...
      if (event.key === 'F2') {
        event.preventDefault();
        if (this.orden) {
          this.router.navigate(['/segundo-faltantes', this.orden]);
        } else {
