...
@Component({
  selector: 'app-primer-faltantes',
  imports: [MatListModule, CommonModule],
  templateUrl: './primer-faltantes.component.html',
  styleUrl: './primer-faltantes.component.css'
})
export class PrimerFaltantesComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.procesosCedisService.primerFaltante(orden).subscribe(resp => {
      this.resultado = resp;
   ...
   @HostListener('document:keydown', ['$event'])
    handleKeyboardEvent(event: KeyboardEvent) {
      if (event.key === '1') {
        ...
        this.router.navigate(['/primer-scaner']);
      }
    }
}
